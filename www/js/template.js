...
MerchantCarousel = function(data){
	...
	$.each(data, function(key, val){
	    ...
	    html +='<ons-ripple color="#a3d200" background="#a3d200"></ons-ripple>';
...
CuisineCarousel = function(data){
	...
	html = '<ons-carousel fullscreen swipeable auto-scroll overscrollable id="carousel" direction="horizontal" item-width="100%">';	
	$.each(data, function(key, val){
	    ...
	    html +='<ons-ripple color="#a3d200" background="#a3d200"></ons-ripple>';
		    html +='<div class="banner">';		
			    ...
			    html +='<div class="header_bg" style="background-image: url('+ "'" + val.featured_image + "'" +');background-position:center;"  >';			       
			...
			html+='<p class="concat_text">' ;			
			 if(!empty(val.services)){               	                                	
	                if(val.services.length>=1){
	                	html+='<ons-row>';
	                	$.each(val.services, function(services_key, services_val){	                			                		
	                		html+='<ons-col  width="60px"><span class="indent">'+services_val+'</span></ons-col>';
	                	});
	                	html+='</ons-row>';
	                }
                }
		...
		html+='<ons-list-item modifier="longdivider" tappable onclick="itemDetails('+ "'"+ val.item_id+"'," + "'" + val.cat_id + "'"  +')"  >';
		  ...
		  html+='<div class="center">';
			    ...
			    if(val.dish_image.length>0){
			      html+='<ons-row>';		    
			         $.each( val.dish_image, function( d_key, d_val ) {      	             
			            html+='<ons-col vertical-align="top" width="50px">';
			                html +='<div class="is-loading">'; 
				            html +='<div class="spinner small"></div>';		
		    ...
		    if (val.prices.length>0){
		    	html+='<span class="list-item__subtitle price">';
...
accountMenu = function(login){
	...
	if (login){
		 ...
		 html+='<ons-list modifier="list_menu">';
		  ...
		  html+='<ons-list-item modifier="chevron" tappable  onclick="showPage(\'blog.html\')" >';
	        html+='<div class="left"><ons-icon icon="ion-pizza" size="22px"></ons-icon></div>';
	        html+='<div class="center">' + t('Блог о еде') +'</div>';
	      html+='</ons-list-item>';
	 ...
	 html+='<ons-list-item modifier="longdivider full_list" tappable >';
	   html+='<ons-row>';
	     html+='<ons-col width="60px" vertical-align="center" >';
	        ...
	        html +='</div>'; 
	     html+='</ons-col>';
	     html+='<ons-col width="160px" vertical-align="top">'+ val.customer_name +'</ons-col>';
